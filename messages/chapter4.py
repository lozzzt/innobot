...
async def fourth_chapter_answer_1(message: types.Message):
    ...
    if(message.text == "Да, не терпится!"):
        ...
        await message.answer("Похоже ты сегодня не в духе. Лови виртуальную поддержку")
        await asyncio.sleep(1)
        await bot.send_sticker(chat_id = message.chat.id, sticker = 'CAACAgIAAxkBAAEFVQdi2SbvgakB-hVLYA6v-ngBBIX7rwAC1AIAAiQ33gHkIRIU_jVelSkE')
        await message.answer("А я постараюсь всё объяснить максимально просто.", reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_block_1(message: types.Message):
    ...
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/goals-p1.png', 'rb'))
    await asyncio.sleep(2)
    ...
    await asyncio.sleep(2)
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/goals-p2.png', 'rb'))
...
async def fourth_chapter_block_2(message: types.Message):
    ...
    await asyncio.sleep(2)
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/goals-p3.png', 'rb'))
...
async def fourth_chapter_answer_3(message: types.Message):
    ...
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/goals-p4.png', 'rb'))
...
async def fourth_chapter_block_3(message: types.Message):
    ...
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/4_smart.png', 'rb'))
...
async def fourth_chapter_block_5(message: types.Message):
    ...
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/4_smart_upr.png', 'rb'), reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_block_6(message: types.Message):
    ...
    await message.answer("""__Перейдем к контролю разных типов сотрудников.__\n
...
• повысишь вероятность того, что задачи будут выполнены качественно и в срок.""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_block_7(message: types.Message):
    ...
    await asyncio.sleep(1)
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/control-process-and-result.png', 'rb'), reply_markup=types.ReplyKeyboardRemove())
    await asyncio.sleep(2)
...
async def fourth_chapter_answer_10(message: types.Message):
    ...
    await message.answer("Р4 достаточно одной точки контроля – примерно за одну треть времени до конца срока. "
    "Профессионалу необходимо напомнить, что задача важна и уточнить, не нужны ли ему какие-либо дополнительные ресурсы. "
    , reply_markup=types.ReplyKeyboardRemove())
    await asyncio.sleep(1)

    await message.answer("Сделал для тебя картинку с распределением точек контроля сотрудника в зависимости от уровня его профессионального развития."
    , reply_markup=types.ReplyKeyboardRemove())
    await asyncio.sleep(1)

    await bot.send_photo(chat_id = message.chat.id, photo = open('media/4_smart_diag.png', 'rb'))
    await asyncio.sleep(1)

    await message.answer("И последний важный момент:  "
    "сотрудников Р1 и Р2 необходимо контролировать в процессе, а с Р3 и Р4 достаточно сверяться по результатам. ", reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_block_11(message: types.Message):
    await message.answer("Давай начнем с Р2. Это разочаровавшиеся ребята с не очень высокой компетентностью. "
    "Они требуют от руководителя очень много внимания и “вложений” времени и сил.", reply_markup=types.ReplyKeyboardRemove())

    await asyncio.sleep(2)

    await bot.send_photo(chat_id = message.chat.id, photo = open('media/motvationp2.png', 'rb'), reply_markup=types.ReplyKeyboardRemove())
    ...
    await bot.send_photo(chat_id = message.chat.id, photo = open('media/motivationP3.png', 'rb'), reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_answer_15(message: types.Message):
    ...
    await message.answer("Спасибо, что поделился своим мнением! "
    "Расскажу, как это вижу я, давай поговорим о нюансах похвалы сотрудников с разным УПР. ", reply_markup=types.ReplyKeyboardRemove())
...
async def fourth_chapter_block_14(message: types.Message):
    ...
    "• Задача сложная, потребуется твой талант (находить подход к людям, нестандартные пути, нужных людей)\n"
...
Этот человек, который сам может сделать все идеально. Его очень важно поощрять, чтобы не демонизировать.""", reply_markup=types.ReplyKeyboardRemove())
    ...
    await message.answer("""Ура!!! Поздравляю тебя! 
Теория уже позади, ответь на несколько моих вопросов, 
чтобы понять, насколько хорошо ты усвоил информацию из этого раздела.""", reply_markup=types.ReplyKeyboardRemove())
...
Сказал, что всегда на связи и готов ответить на все вопросы.""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=types.ReplyKeyboardRemove())
...
Как ты думаешь, совершил ли управленческую ошибку Роман?""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=keyboard)
...
Как ты думаешь, правильно ли Роман поставил задачи и обозначил точки контроля?""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=keyboard)
...
async def fourth_chapter_answer_18(message: types.Message):
    ...
    if(message.text == "Да, так как Аня Р1"):
        await message.answer("Согласен! \nДля новичка это самый хороший вариант", reply_markup=types.ReplyKeyboardRemove())
        counter.increase_counter()
    else:
        await message.answer("Не совсем согласен. \nДля новичка это как раз самый хороший вариант", reply_markup=types.ReplyKeyboardRemove())
...
Как Роману стоит мотивировать Анну?""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=keyboard)
...
async def fourth_chapter_answer_19(message: types.Message):
    ...
    keyboard.add("Ограничение по времени")
    keyboard.add("Достижимость и реалистичность")
    keyboard.add("Все ответы неверны")

    await message.answer("""*Вопрос 4.* 
Какие из перечисленных критериев постановки целей по SMART можно упустить, ставя цели Анне?""", parse_mode=types.ParseMode.MARKDOWN, reply_markup=keyboard)

    await Progress4.msg_20.set()   

async def fourth_chapter_answer_20(message: types.Message, state: FSMContext):
    await asyncio.sleep(1)

    if(message.text == "Все ответы неверны"):
        await message.answer("Абсолютно точно!\n"
        "Ставя цели для Р1, лучше использовать все критерии SMART.\n" 
        "Иногда можно упустить только реалистичность (в связи с тем, что мы не знаем всего потенциала сотрудника)",
        reply_markup=types.ReplyKeyboardRemove())
        counter.increase_counter()
    else:
        await message.answer("Увы, нет! \n"
        "Ставя цели для Р1 лучше использовать все критерии SMART. \n"
        "Иногда можно упустить только реалистичность (в связи с тем, что мы не знаем всего потенциала сотрудника) ",
        reply_markup=types.ReplyKeyboardRemove())
    ...
    await message.answer("ЕЕЕ! \n"
    "Ты ответил на все вопросы! \n"
    "И у тебя {0} {1} из 4.".format(correct_answers, word),
    reply_markup=types.ReplyKeyboardRemove())
    await fourth_chapter_block_16(message, state)

async def fourth_chapter_block_16(message: types.Message, state: FSMContext):
    await state.finish()
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    keyboard.add("5. Общее тестирование")
    await message.answer("У тебя остался всего один маленький шажок до финала – итоговый тест!", reply_markup=keyboard)